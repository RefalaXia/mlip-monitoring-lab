...
REQUEST_COUNT = Counter(
    ...
    'Recommendation Request Count',           # Metric description
    ['http_status']                           # Labels
)
...
def main():
    ...
    for message in consumer:
        ...
        if 'recommendation request' in values[2]:
            ...
            print(values)
            status = values[3].strip().split(' ')[1]  # Splits by 'status ' and takes the numeric part
            REQUEST_COUNT.labels(status).inc()  # status holds the HTTP status code extracted
